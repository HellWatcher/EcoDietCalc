...
public static class PlanRenderer
{
    ...
    // ── UILink tooltip path (returns LocString with interactive food/source links) ──
    ...
    private static List<string> BuildTooltipItemTags(ItemGroup group)
    {
        ...
        if (MathF.Abs(group.TotalVarietyDeltaPp) > VarietyDeltaThreshold)
            tags.Add(Text.Color(RT.TagVariety, $"variety {FormatSigned(group.TotalVarietyDeltaPp)}pp"));
        if (MathF.Abs(group.TotalTastinessDeltaPp) > TastinessDeltaThreshold)
            tags.Add(Text.Color(RT.TagTaste, $"taste {FormatSigned(group.TotalTastinessDeltaPp)}pp"));
    ...
    /// </summary>
    private static LocString ResolveSourceLink(SourceInfo source)
    {
        if (source.WorldObj != null)
        {
            ...
            {
                // WorldObject may have been destroyed between discovery and render
            }
        }
    ...
    /// </summary>
    private static LocString ResolveFoodLink(string name, DiscoveryResult? discovery)
    {
        if (discovery != null)
        {
            var candidate = FindCandidateByName(discovery.Available, name);
            if (candidate?.FoodType != null)
            {
                ...
                {
                    // Item resolution failed — fall through to text fallback
                }
            }
        }
